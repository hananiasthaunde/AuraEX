...
// Devolve false em vez de engolir a falha: num sistema de ficheiros só de
// leitura (Vercel) a gravação nunca acontece, e quem chama tem de conseguir
// dizer isso ao utilizador em vez de responder sucesso.
export const PERSIST_ERROR = 'Não foi possível gravar a alteração. Neste ambiente o sistema de ficheiros é apenas de leitura, por isso a alteração não ficou guardada.';
...
function atomicWriteJson(file, value) {
  try {
    ...
    console.warn('Persistência em disco falhou:', err.message);
    return false;
  }
}
...
export function changePassword(userId, oldPassword, newPassword) {
  ...
  if (!atomicWriteJson(USERS_FILE, doc)) return { ok: false, error: PERSIST_ERROR };
...
// Impede que a instalação fique sem forma de entrar como administrador.
function isLastActiveAdmin(doc, user) {
  if (user.role !== 'admin' || user.active === false) return false;
  return doc.users.filter(item => item.role === 'admin' && item.active !== false).length <= 1;
}

export function listUsers() {
  return getUsersDocument().users
    .map(user => ({
      id: user.id,
      name: user.name,
      email: user.email,
...
export function createUser({ name, email, password, role = 'user' } = {}) {
  ...
  if (cleanName.length < 2) return { ok: false, error: 'Indique o nome do utilizador.' };
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(cleanEmail)) return { ok: false, error: 'E-mail inválido.' };
  if (typeof password !== 'string' || password.length < 12) return { ok: false, error: 'A senha deve ter pelo menos 12 caracteres.' };
  if (!VALID_ROLES.includes(role)) return { ok: false, error: 'Papel inválido.' };

  const doc = getUsersDocument();
  if (doc.users.some(user => normalizeEmail(user.email) === cleanEmail)) {
...
export function setUserActive(userId, active, actorId) {
  if (userId === actorId && !active) return { ok: false, error: 'Não pode desativar a sua própria conta.' };
  const doc = getUsersDocument();
  const user = doc.users.find(item => item.id === userId);
  if (!user) return { ok: false, error: 'Utilizador não encontrado.' };
  if (!active && isLastActiveAdmin(doc, user)) return { ok: false, error: 'Esta é a única conta de administrador ativa.' };
...
export function setUserPassword(userId, newPassword) {
  if (typeof newPassword !== 'string' || newPassword.length < 12) {
    return { ok: false, error: 'A senha deve ter pelo menos 12 caracteres.' };
  }
  const doc = getUsersDocument();
  const user = doc.users.find(item => item.id === userId);
  if (!user) return { ok: false, error: 'Utilizador não encontrado.' };
  ...
  user.updatedAt = new Date().toISOString();
  if (!atomicWriteJson(USERS_FILE, doc)) return { ok: false, error: PERSIST_ERROR };
  // O passwordFingerprint muda, portanto as sessões abertas deste utilizador caem.
  return { ok: true, user: cleanUser(user) };
}

export function deleteUser(userId, actorId) {
  if (userId === actorId) return { ok: false, error: 'Não pode remover a sua própria conta.' };
  const doc = getUsersDocument();
  const index = doc.users.findIndex(item => item.id === userId);
  if (index < 0) return { ok: false, error: 'Utilizador não encontrado.' };
  if (isLastActiveAdmin(doc, doc.users[index])) return { ok: false, error: 'Esta é a única conta de administrador ativa.' };
  const [removed] = doc.users.splice(index, 1);
  if (!atomicWriteJson(USERS_FILE, doc)) return { ok: false, error: PERSIST_ERROR };

  // Os tokens do utilizador removido deixariam de ter dono; revoga-os.
  const tokens = getTokensDocument();
  let changed = false;
  ...
  if (changed) atomicWriteJson(TOKENS_FILE, tokens);

  return { ok: true, user: cleanUser(removed) };
}
...
export function createPat(userId, { name = 'Token pessoal', scopes = ['auraex:read'], expiresInDays = 365 } = {}) {
  ...
  if (!atomicWriteJson(TOKENS_FILE, doc)) return { ok: false, error: PERSIST_ERROR };
  return { ok: true, token: rawToken, record: { ...record, tokenHash: undefined } };
}

export function revokePat(userId, tokenId) {
  const doc = getTokensDocument();
  const record = doc.tokens.find(token => token.id === tokenId && token.userId === userId);
  if (!record) return { ok: false, error: 'Token não encontrado.' };
